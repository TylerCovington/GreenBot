...
image_size = (32,5534) # Tuple size of image
...
def create_grad_array(image_array):
	...
	# gamma correction (uncomment if needed)
	...
	gradient_magnitude_stack = np.dstack((new_grad,new_mag))
...
def create_grad_arrays_from_file(file_name):
	...
	with open(file_name, 'rb') as f:
		...
		for count, row in enumerate(reader):
			...
			labels.append(row[0])
			array = np.reshape(array,(5532,32))
